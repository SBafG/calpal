// Variation 3 — Skandinavisk modernism (parameterized + polished)
// Accepts a `tweaks` prop that controls accent, density, season palette, etc.

const SkandinaviskFinal = ({ data, tweaks }) => {
  const t = tweaks;

  // ── Color system ──────────────────────────────────────────────────────────
  // Season presets — both background tint and a default accent.
  const SEASON = {
    var:    { bg: '#fafaf7', tint: '#f4f6ee', name: 'Vår',    label: 'Blomstertid',  poem: 'Björklövet slår ut, näktergalen sjunger.', accent: '#c44a2a' },
    sommar: { bg: '#fbfaf3', tint: '#f7f3e2', name: 'Sommar', label: 'Hundens dagar',poem: 'Solen står som högst, ängarna gula.',     accent: '#d68a18' },
    host:   { bg: '#fbf8f4', tint: '#f3ece1', name: 'Höst',   label: 'Skördetid',    poem: 'Mikaeli vänder bladen — och hjärtat med.',  accent: '#8a3a1a' },
    vinter: { bg: '#f7f9fa', tint: '#ecf0f3', name: 'Vinter', label: 'Mörkertid',    poem: 'Knut visar julen ut, snön ligger djupt.',   accent: '#2a4a6a' },
  };
  const season = SEASON[t.season] || SEASON.var;

  // User can override accent or follow the season default.
  const accent = t.accentMode === 'season' ? season.accent : t.accent;
  ...
  // Density scaling
  const D = ({
    compact: { padX: 40, dayPadY: 10, daySize: 32, dayMin: 70, namesize: 10.5, gap: 8,  sidegap: 28 },
    regular: { padX: 56, dayPadY: 14, daySize: 38, dayMin: 86, namesize: 11,   gap: 12, sidegap: 36 },
    comfy:   { padX: 72, dayPadY: 18, daySize: 44, dayMin: 102,namesize: 12,   gap: 16, sidegap: 44 },
  })[t.density] || {};
  ...
  const DayCell = ({ d, isToday }) => {
    ...
    return (
      <div data-comment-anchor={isToday ? 'day-today' : undefined} style={{
        position: 'relative',
        padding: `${D.dayPadY}px 0 ${D.dayPadY + 4}px`,
        minHeight: D.dayMin,
        borderTop: `1px solid ${isToday ? C.ink : C.rule}`,
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
          <span style={{
            ...
            fontWeight: 300,
            fontSize: isToday ? D.daySize + 8 : D.daySize,
            color: isHoliday ? C.accent : C.ink,
            lineHeight: 0.9, letterSpacing: '-0.02em',
          }}>{d.day}</span>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
            {isHoliday && !isToday && <span style={{ width: 6, height: 6, borderRadius: 999, background: C.accent }}/>}
        ...
        <div style={{
          marginTop: 8, fontSize: D.namesize, lineHeight: 1.35,
          color: isHoliday ? C.accent : C.ink2, letterSpacing: '0.02em',
        }}>{d.name}</div>
        {d.holiday && (
  ...
  // Bondepraktikan + dagens ord can shift per season
  const bondePicked = data.BONDEPRAKTIKAN[
    { var: 5, sommar: 7, host: 10, vinter: 1 }[t.season] || 5
  ];

  return (
    <div data-screen-label="01 Cal·Pal Idag" style={{
      width: '100%', minHeight: '100vh', background: C.bg, color: C.ink,
      fontFamily: '"Inter", "Helvetica Neue", system-ui, sans-serif',
      boxSizing: 'border-box',
    }}>
      {/* Top strip */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: `18px ${D.padX}px`, borderBottom: `1px solid ${C.rule}` }}>
      ...
      <div style={{ padding: `0 ${D.padX}px` }}>
        {/* Masthead */}
        {t.masthead !== 'none' && (
          <div style={{ padding: t.masthead === 'compact' ? '32px 0 24px' : '52px 0 40px', borderBottom: `1px solid ${C.rule}` }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', alignItems: 'flex-end', gap: 32 }}>
              <h1 style={{
                fontFamily: '"Newsreader", serif',
                fontWeight: 300,
                fontSize: t.masthead === 'compact' ? 96 : 192,
                margin: 0, lineHeight: 0.84,
                letterSpacing: '-0.04em', color: C.ink,
              }}>
                Cal<span style={{ fontStyle: 'italic', color: C.accent }}>·</span>Pal
              </h1>
              <div style={{ textAlign: 'right', paddingBottom: t.masthead === 'compact' ? 6 : 14 }}>
                <div style={{ fontSize: 11, letterSpacing: '0.28em', textTransform: 'uppercase', color: C.ink3 }}>
                  nummer cxlvii &nbsp;·&nbsp; {season.name.toLowerCase()}
                </div>
                <div style={{ fontFamily: 'Newsreader, serif', fontStyle: 'italic', fontSize: 22, color: C.ink2, marginTop: 6, lineHeight: 1.3, maxWidth: 280 }}>
        ...
        {/* Today stats strip */}
        {t.showStats && (
          <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr 1fr 1fr', borderBottom: `1px solid ${C.rule}` }}>
            {[
              { l: 'Idag', big: 'onsdag', sub: `27 maj · ${season.label.toLowerCase()}`, accent: true, italic: true },
              { l: 'Namnsdag', big: 'Beda', sub: '· Blenda' },
              { l: 'Dag av året', big: '147', sub: 'av 365' },
              { l: 'Dagar kvar', big: '218', sub: 'till nyår' },
            ].map((c, i) => (
              <div key={c.l} style={{
                ...
                <div style={{
                  ...
                  fontSize: 58, color: C.ink, lineHeight: 0.95, marginTop: 12, letterSpacing: '-0.025em',
                }}>{c.big}</div>
        ...
        {/* Main */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: D.sidegap + 20, padding: '48px 0 32px' }}>
          {/* Calendar column */}
          <div>
            <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 28 }}>
              <div>
                ...
                <h2 style={{
                  ...
                  fontSize: 108, margin: '4px 0 0', lineHeight: 0.9, letterSpacing: '-0.03em',
                }}>maj</h2>
              </div>
              <div style={{ textAlign: 'right' }}>
                ...
                <div style={{ fontSize: 12, color: C.ink2, fontFamily: 'Newsreader, serif', fontStyle: 'italic', maxWidth: 220, lineHeight: 1.5 }}>
                  Tre helgdagar, två klämdagar. {season.poem}
                </div>
              </div>
            </div>
            ...
            <div style={{
              display: 'grid',
              gridTemplateColumns: `${t.showWeeks ? '28px ' : ''}repeat(7, 1fr)`,
              gap: D.gap, paddingBottom: 8, borderBottom: `2px solid ${C.ink}`,
            }}>
              {t.showWeeks && <div></div>}
              {wd.map((w, i) => (
                <div key={i} style={{
          ...
          <aside style={{ display: 'flex', flexDirection: 'column', gap: 36 }}>
            ...
            <div>
              ...
              <svg width="100%" height="92" viewBox="0 0 280 92">
                ...
                {(() => {
                  const tt = 0.62, x = 14 + 252 * tt, y = 78 - Math.sin(tt * Math.PI) * 76;
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                ...
                <div>
                  <div style={{ fontFamily: 'Newsreader, serif', fontStyle: 'italic', fontSize: 24, color: C.ink, lineHeight: 1 }}>{season.label}</div>
...
window.SkandinaviskFinal = SkandinaviskFinal;
